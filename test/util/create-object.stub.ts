...
type StubObjMethodNames<T = undefined> = T extends undefined
...
export function createStubObj<T>(
  methodNames: StubObjMethodNames<T>,
  sandbox?: SinonSandbox,
): CustomSinonStubbedInstance<T> {
  let _stub;
  if (sandbox) {
    _stub = sandbox.stub;
  } else {
  ...
  (methodNames as any).forEach(
    methodName => (stubbedInstance[methodName] = _stub()),
  );
